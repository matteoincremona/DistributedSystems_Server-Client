...
def main():
    ...
    desired_id = int(input("Type your ID (only int numbers): "))
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        if handshake.error:
            print(f'!!!The Requested ID {desired_id} is already taken :(\n-> Assigned ID: {handshake.id}')
            print(f'Connected to the server with re-assigned ID {handshake.id}')
        else:
            print(f'Requested ID {desired_id} available :)\n-> Assigned ID: {handshake.id}')
